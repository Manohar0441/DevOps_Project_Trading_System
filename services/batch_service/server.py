...
_SERVICE = "batch-service"


class BatchRequestHandler(MetricsMixin, BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path == "/health":
            send_json(self, {"status": "ok", "service": _SERVICE, "max_workers": self.max_workers})
            return
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            # ── record metrics ─────────────────────────────────────────────
            successes = sum(1 for r in result.get("results", []) if r.get("status") == "ok")
            errors = len(jobs) - successes
            status_label = "success" if errors == 0 else ("partial" if successes > 0 else "error")
            BATCH_JOBS_TOTAL.labels(status=status_label).inc()
            BATCH_JOB_DURATION_SECONDS.observe(duration)
            if successes:
                BATCH_STOCKS_PROCESSED_TOTAL.labels(status="success").inc(successes)
            if errors:
                BATCH_STOCKS_PROCESSED_TOTAL.labels(status="error").inc(errors)

            send_json(self, result)
        except ValueError as exc:
            logger.warning("Invalid batch request: %s", exc)
            BATCH_JOBS_TOTAL.labels(status="error").inc()
